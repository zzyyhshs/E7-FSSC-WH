...
class test_A_SXSQD1(mytest.MyTest):
	"""测试单据-费用报销组（多币种）-8.04-报销单-有申请"""
	@unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
	def test_DBZ_BXDY1_01(self):
		"""测试单据-费用报销组（多币种）-8.04-报销单-有申请-本位币为人民币-币种一致"""
		#初始化测试对象
		fsscTest = SubmitToBillPage.SubmitToBillPage(self.dr)
		#准备测试数据
		billName = "8.04-报销单-有申请"
		fillPerson = "填单用户001"
		approvalModel = "Auto"
		testCaseFile = "费用报销组（多币种）\\8.04-报销单-有申请-01.xls"
	...
	@unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
	def test_DBZ_BXDY1_02(self):
		"""测试单据-费用报销组（多币种）-8.04-报销单-有申请-本位币为人民币-币种不一致"""
		# 初始化测试对象
		fsscTest = SubmitToBillPage.SubmitToBillPage(self.dr)
		# 准备测试数据
		billName = "8.04-报销单-有申请"
		fillPerson = "填单用户001"
		approvalModel = "Auto"
		testCaseFile = "费用报销组（多币种）\\8.04-报销单-有申请-02.xls"
		testCaseData = BillTestData(testCaseFile)
		# 打开系统
		fsscTest.openSystem(globalparam.system_address)
		fsscTest.login(fillPerson)
	...
	@unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
	def test_DBZ_BXDY1_03(self):
		"""测试单据-费用报销组（多币种）-8.04-报销单-有申请-本位币为美元-币种一致"""
		#初始化测试对象
		fsscTest = SubmitToBillPage.SubmitToBillPage(self.dr)
		#准备测试数据
		billName = "8.04-报销单-有申请"
		fillPerson = "填单用户007"
		approvalModel = "Auto"
		testCaseFile = "费用报销组（多币种）\\8.04-报销单-有申请-03.xls"
		testCaseData = BillTestData(testCaseFile)
		#打开系统
		fsscTest.openSystem(globalparam.system_address)
		fsscTest.login(fillPerson)
	...
	@unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
	def test_DBZ_BXDY1_04(self):
		"""测试单据-费用报销组（多币种）-8.04-报销单-有申请-本位币为美元-币种不一致"""
		# 初始化测试对象
		fsscTest = SubmitToBillPage.SubmitToBillPage(self.dr)
		# 准备测试数据
		billName = "8.04-报销单-有申请"
		fillPerson = "填单用户007"
		approvalModel = "Auto"
		testCaseFile = "费用报销组（多币种）\\8.04-报销单-有申请-04.xls"
		testCaseData = BillTestData(testCaseFile)
		# 打开系统
		fsscTest.openSystem(globalparam.system_address)
		fsscTest.login(fillPerson)
