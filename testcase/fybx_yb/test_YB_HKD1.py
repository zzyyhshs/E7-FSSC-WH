...
class test_YB_HKD1(mytest.MyTest):
	"""测试单据-费用报销组（原币）-13.07-还款单"""
	@unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
	def test_YB_HKD1_01(self):
		"""测试单据-费用报销组（原币）-13.07-还款单-本位币人民币"""
		#初始化测试对象
		...
		fillPerson = "填单用户001"
		approvalModel = "Auto"
		testCaseFile = "费用报销组（原币）\\13.07-还款单-01.xls"
	...
	@unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
	def test_YB_HKD1_02(self):
		"""测试单据-费用报销组（原币）-13.07-还款单-本位币美元"""
		#初始化测试对象
		fsscTest = SubmitToBillPage.SubmitToBillPage(self.dr)
		#准备测试数据
		billName = "13.07-还款单"
		fillPerson = "填单用户007"
		approvalModel = "Auto"
		testCaseFile = "费用报销组（原币）\\13.07-还款单-02.xls"
		testCaseData = BillTestData(testCaseFile)
		#打开系统
		fsscTest.openSystem(globalparam.system_address)
		fsscTest.login(fillPerson)
