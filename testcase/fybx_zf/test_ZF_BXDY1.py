...
class test_ZF_BXDY1(mytest.MyTest):
	"""测试单据-费用报销组（支付币种）-12.04-报销单-有申请"""
	@unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
	def test_ZF_BXDY1_01(self):
		"""测试单据-费用报销组（支付币种）-12.04-报销单-有申请-本位币为人民币"""
		#初始化测试对象
		fsscTest = SubmitToBillPage.SubmitToBillPage(self.dr)
		#准备测试数据
		billName = "12.04-报销单-有申请"
		fillPerson = "填单用户001"
		approvalModel = "Auto"
		testCaseFile = "费用报销组（支付币种）\\12.04-报销单-有申请-01.xls"
	...
	@unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
	def test_ZF_BXDY1_02(self):
		"""测试单据-费用报销组（支付币种）-12.04-报销单-有申请-本位币为美元"""
		#初始化测试对象
		fsscTest = SubmitToBillPage.SubmitToBillPage(self.dr)
		#准备测试数据
		billName = "12.04-报销单-有申请"
		fillPerson = "填单用户007"
		approvalModel = "Auto"
		testCaseFile = "费用报销组（支付币种）\\12.04-报销单-有申请-02.xls"
		testCaseData = BillTestData(testCaseFile)
		#打开系统
		fsscTest.openSystem(globalparam.system_address)
		fsscTest.login(fillPerson)
