...
class test_BXDW(mytest.MyTest):
    """测试单据-费用报销组（系统内置）-1.3-报销单（无申请)"""
    @unittest.skipUnless(globalparam.usecase_run_mode >= 1, "")
    def test_BXDW_01(self):
        """测试单据-费用报销组（系统内置）-1.3-报销单（无申请)_Auto模式"""
        #初始化测试对象
        fsscTest = SubmitToBillPage.SubmitToBillPage(self.dr)
        #准备测试数据
        billName = "1.3-报销单（无申请)"
        fillPerson = "填单用户001"
        approvalModel = "Auto"
        testCaseFile = "费用报销组（系统内置）\\1.3-报销单（无申请).xls"
